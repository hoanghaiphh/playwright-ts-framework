...
export function mapUser(email: string, firstName: string, lastName: string, company: string): UserRecord {
    return {
        Email: email,
        FirstName: firstName,
        LastName: lastName,
...
export async function getUserFromDatabaseByEmail(email: string): Promise<UserRecord | null> {
    const query =
...
export async function deleteUserFromDatabaseByEmail(email: string): Promise<void> {
